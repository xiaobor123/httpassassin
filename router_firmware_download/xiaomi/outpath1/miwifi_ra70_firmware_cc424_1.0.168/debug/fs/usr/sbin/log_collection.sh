...
WHC_LOG="/tmp/whc.log"   ### for D01/RM1800/RA70 xqwhc
GZ_LOGS=""
...
sed -i "/^nv_wifi_pwd1=/cnv_wifi_pwd1=******" $NVRAM_FILE_PATH
...
if [ "$hardware" = "R1D" ] || [ "$hardware" = "R2D" ]; then
    ...
    [ $? = 0 ] && list="$list 14"
    [ "$netmode" = "whc_cap" -o "$netmode" = "lanapmode" -a "$capmode" = "ap" -o "$netmode" = "whc_re" ] && {
        ...
        for bh_radio in $bh_radio_list; do
            ...
            if [ -n "$bh_vap_if" ] && [ "$bh_vap_if" != "wl0" ] && [ "$bh_vap_if" != "wl1" ] && [ "$bh_vap_if" != "wl2" ]; then
    ...
    for i in $list; do
        ...
        log_exec "hostapd_cli -p /var/run/hostapd-wifi$dev_id -i wl$i get_config | grep -v \"passphrase=\""
    done
