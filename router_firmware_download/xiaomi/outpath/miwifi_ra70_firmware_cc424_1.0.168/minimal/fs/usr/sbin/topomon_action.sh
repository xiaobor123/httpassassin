...
topomon_wifi_ap_if_down() {
	local ap_iface=$1
	local bh_5g_ap=$(uci -q get misc.backhauls.backhaul_5g_ap_iface)
	local device="wifi2"

	[ "$ap_iface" == "$bh_5g_ap" ] && device="wifi0"

	if [ -n "$ap_iface" ]; then
		log "Interface $ap_iface Brought down"
		hostapd_cli -i $ap_iface -p /var/run/hostapd-$device disable
	fi
}

topomon_wifi_ap_if_up() {
	local ap_iface=$1
	local bh_5g_ap=$(uci -q get misc.backhauls.backhaul_5g_ap_iface)
	local device="wifi2"
	...
	[ "$ap_iface" == "$bh_5g_ap" ] && device="wifi0"
	if [ -n "$ap_iface" ]; then
		log "Interface $ap_iface Brought up"
		hostapd_cli -i $ap_iface -p /var/run/hostapd-$device enable
	fi
}
...
set_backhaul_ap_aplimit() {
	...
	if [ $hop_count = "0" ]; then
		...
		local device_bh=$(uci -q get misc.wireless.if_${bh_tag})
		hostapd_cli -i $bh_wlan_iface -p /var/run/hostapd-${device_bh} list_sta | while read line;do sleep 1;cfg80211tool $bh_wlan_iface kickmac $line;done
	fi
}
...
topomon_link_update() {
	...
	if [ "$bh_type" = "1" ]; then
		...
		local bh_sta_iface=$(uci -q get misc.backhauls.backhaul_${bh_tag}_sta_iface)
...
topomon_topo_update() {
	local bh_type="$1"
	local port_name="$2"
	local bh_wlan_iface=$(uci -q get misc.backhauls.backhaul_${bh_tag}_ap_iface)
...
topomon_init() {
	...
	if [ "$bh_type" = "wired" ]; then
		...
		local bh_sta_iface_sec=$(uci -q get misc.backhauls.backhaul_5g_sta_iface)
		[ "5g" = "$bh_tag" ] && bh_sta_iface_sec=$(uci -q get misc.backhauls.backhaul_game_sta_iface)
		topomon_wifi_if_down $bh_sta_iface_sec
	fi
...
topomon_update_re_wifi_param() {
	local backhaul_5g_ap_iface=$(uci -q get misc.backhauls.backhaul_${bh_tag}_ap_iface)
...
case "$1" in
	...
	;;
	wifi_ap_if_up)
	topomon_wifi_ap_if_up "$2"
	;;
	wifi_ap_if_down)
	topomon_wifi_ap_if_down "$2"
	;;
	set_connect_bssid)
