...
guest_ip_make()
{
    ...
    startip="$(lua /usr/sbin/guestwifi_mkip.lua "$new_ip" "$lan_mask", "startip")"
    endip="$(lua /usr/sbin/guestwifi_mkip.lua "$new_ip" "$lan_mask", "endip")"
    ...
    then
        ...
        # netmask="255.255.0.0"
        netmask=$lan_mask
        echo "set guest lan_mask $netmask"
    fi
...
then
    ...
    uci -q batch <<-EOF >/dev/null
        ...
        set dhcp.${network_name}.startip=$startip
        set dhcp.${network_name}.endip=$endip
        commit dhcp
EOF
