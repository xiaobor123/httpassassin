...
get_scan_result()
{
	...
	local cchannel_line=$(grep -wn "${cchannel})" ${ACSREPORT_FILE} | awk -F ':' '{print $1}')
	local firstchannel_line=$(grep -wn "1)" ${ACSREPORT_FILE} | awk -F ':' '{print $1}')
	#echo "cchannel_line=${cchannel_line}, firstchannel_line=${firstchannel_line}"
