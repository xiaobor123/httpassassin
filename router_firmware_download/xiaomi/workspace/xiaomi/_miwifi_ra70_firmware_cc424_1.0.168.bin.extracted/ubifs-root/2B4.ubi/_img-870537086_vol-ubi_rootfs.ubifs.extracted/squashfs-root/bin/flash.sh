...
fw_channel=`mkxqimage -V "$1"|grep CHANNEL|grep stable`
...
system_channel=`bootinfo|grep CHANNEL|grep release`
if [ -n "$fw_channel" -a -n "$system_channel" -a "$fw_channel" != "$system_channel" ];then
	upgrade_done_set_flags $1 1 $3
else	
