...
char **get_qemu_argv(u8 *own_loc, u8 **target_path_p, int argc, char **argv) {
  ...
  char **new_argv = ck_alloc(sizeof(char *) * (argc + 8));
  if (unlikely(!new_argv)) { FATAL("Illegal amount of arguments specified"); }

  char *qemu_path = find_afl_binary(own_loc, "afl-qemu-trace");
  char *execve_arg = NULL;
  int ret = asprintf(&execve_arg, "%s -hackbind -hackproc", "/qemu-static");
  if (unlikely(ret <= 0)) {
    FATAL("Fail to allocate memory for execve_arg");
  }
